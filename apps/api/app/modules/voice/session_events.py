...
from app.shared.models import ConversationTurn, SessionEvent

log = logging.getLogger("aipal.session_events")
RETENTION_DAYS = 30
...
async def safe_record_event(
    ...
    *,
    ...
    """Never fail the caller (e.g. audio_turn) if observability write fails."""
    try:
        await record_event(
            db,
            user_id,
        ...
        log.exception("session_event_write_failed type=%s session=%s", event_type, session_id)
